...
def bar(
    ...
    if isinstance(shap_values, Explanation):
        ...
        assert isinstance(
            shap_values, dict
        ), "You must pass an Explanation object, Cohorts object, or dictionary to bar plot!"
    ...
    for i in range(len(cohort_exps)):
        ...
        assert isinstance(
            cohort_exps[i], Explanation
        ), "The shap_values paramemter must be a Explanation object, Cohorts object, or dictionary of Explanation objects!"
        assert (
            cohort_exps[i].shape == cohort_exps[0].shape
        ), "When passing several Explanation objects they must all have the same shape!"
    ...
    for exp in cohort_exps:
        for op in exp.op_history:
            ...
            ):  # see if this if the first op to collapse the instances
    ...
    for i in range(len(values)):
        ...
        pl.barh(
            ...
            align="center",
            color=[
                colors.blue_rgb if values[i, feature_inds[j]] <= 0 else colors.red_rgb
    ...
    pl.yticks(
        ...
        yticklabels + [l.split("=")[-1] for l in yticklabels],
        fontsize=13,
    )
...
def bar_legacy(shap_values, features=None, feature_names=None, max_display=None, show=True):
    ...
    pl.barh(
        ...
        align="center",
        color=[
            colors.red_rgb if shap_values[feature_inds[i]] > 0 else colors.blue_rgb
