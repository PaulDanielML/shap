...
def waterfall(shap_values, max_display=10, show=True):
    ...
    for i in range(num_individual):
        ...
        if features is None:
            ...
            yticklabels[rng[i]] = (
                format_value(features[order[i]], "%0.03f") + " = " + feature_names[order[i]]
            )
    ...
    ytick_pos = list(range(num_features)) + list(
        np.arange(num_features) + 1e-8
    )  # The 1e-8 is so matplotlib 3.3 doesn't try and collapse the ticks
    pl.yticks(
        ytick_pos, yticklabels[:-1] + [l.split("=")[-1] for l in yticklabels[:-1]], fontsize=13
    )
    ...
    ax3.set_xticks(
        [base_values + values.sum(), base_values + values.sum() + 1e-8]
    )  # The 1e-8 is so matplotlib 3.3 doesn't try and collapse the ticks
    ax3.set_xticklabels(
...
def waterfall_legacy(
    expected_value, shap_values=None, features=None, feature_names=None, max_display=10, show=True
):
    """Plots an explantion of a single prediction as a waterfall plot.
    ...
    ax2.set_xticklabels(
        ["\n$E[f(X)]$", "\n$ = " + format_value(expected_value, "%0.03f") + "$"],
        fontsize=12,
        ha="left",
    )
    ax2.spines["right"].set_visible(False)
